...
public boolean insertPoll(PollListBean plBean, PollItemBean piBean) {
	...
	try {
		...
		if(cnt==1) { // 정상적인 tblPollList 저장
			...
			pstmt = con.prepareStatement(sql);
			int listNum = getMaxNum(); //방금 저장된 리스트의 num 값
			String item[] = piBean.getItem(); //받은 리스트가 각각의 인스턴스가 되어야한다.
			for (int i = 0; i<item.length; i++) {
				if(item[i] == null || item[i].trim().length()==0) //입력이 없거나 공백이면, 앞의 것들만 입력되고 나머지는 입력이 되지 않는 상태로 만든다.
					break;
...
public PollListBean getPoll(int num) {
    ...
    try {
        ...
        pool.freeConnection(con, pstmt, rs); // ResultSet도 반환해야 합니다.
    }
    return plbean;
}
...
//Max Item Count : 아이템 중에  가장 높은 투표의 아이템 Count
//sql = "select item, count from tblPollItem where listNum=?";
