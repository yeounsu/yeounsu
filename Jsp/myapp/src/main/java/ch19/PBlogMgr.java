...
public class PBlogMgr {
	...
	//Random PMember List : 본인을 제외한 5명의 리스트
	...
	//PBlog Delete : 파일까지 삭제 (getPhoto 메소드 이용)
	//UtilMgr.delete 메소드 이용
	//PBlogDeleteServlet.java()
	public void deletePBlog(int num) {
		...
		try {
			...
			String gp = getPhoto(num);
			UtilMgr.delete(SAVEFOLDER + gp);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			pool.freeConnection(con, pstmt);
